...
# _________________________________________________________________________________________
def processo(varBarra, passo, df_origem, numero_registros_origem, df_destino, numero_registros_destino, df_saida, endereco_arquivo_saida, nome_planilha_saida):
    point = 0
    key = 0
    cont = 0
        ...
        if df_origem.iat[i, 6] != 'ENDEREÇO NÃO ENCONTRADO' and df_origem.iat[i, 8] != 'POINT EMPTY':
            ...
            for j in range(0, numero_registros_destino, 1):
                ...
                if df_destino.iat[j, 6] != 'ENDEREÇO NÃO ENCONTRADO' and df_destino.iat[j, 8] != 'POINT EMPTY':
                    ...
                    # ____________________________________________________________________________________________________________
                    cont_etapas += 1
                    etapa = int((cont_etapas * 100)/total)
                    varBarra.set(etapa)
                    root.update()
                    #sleep(0.1)
...
if __name__ == '__main__':
    ...
    # ____________________________________________________________________________________________________________
    diretorio = 'C:/Users/acmdo/Meu Drive/PYTHON_Coordenadas/DASA/'
    final_arquivo = '.xlsx'
    nome_planilha = 'cep_endereços_coordenadas'

    arquivo_origem = 'Vidas'
    ...
    if os.path.isfile(endereco_arquivo_origem) and os.path.isfile(endereco_arquivo_destino):
        ...
        p_print = '\n   Tempo utilizado nesta etapa: {:2} dias, {:2} h, {:2} min, {:2} s, {:1} décimos, {:1} centésimos, {:.3f} milésimos'.format(dias, horas, minutos, segundos, dec, cent, mile)
        print(p_print)
        # ____________________________________________________________________________________________________________
        ...
        print(p_titulo1, p_titulo2, p_titulo1)
        # ____________________________________________________________________________________________________________
        #   CRIA NA TELA UMA BARRA PARA ACOMPANHAR A EXECUÇÃO
        # ____________________________________________________________________________________________________________        
        # https://www.pythontutorial.net/tkinter/tkinter-progressbar/
        root = Tk()
        root.title('Calculando as distâncias')
        root.geometry('1200x60+40+700')

        varBarra = DoubleVar()
        varBarra.set(0)

        # Criando o Label para a barra de progresso 
        barra_progresso = ttk.Progressbar(root, variable=varBarra, maximum=100)
        barra_progresso.place(x=100, y=10, width=1000, height=40)

        processo(varBarra, n_processos, df_origem, numero_registros_origem, df_destino, numero_registros_destino, df_saida, endereco_arquivo_saida, nome_planilha_saida)
        ...
        dif = final - inicio
        dias, horas, minutos, segundos, dec, cent, mile = transforma_segundos(dif)
        p_print = '\n   Tempo total de processamento: {:2} dias, {:2} h, {:2}, min, {:2} s, {:1} décimos, {:1} centésimos, {:.3f} milésimos'.format(dias, horas, minutos, segundos, dec, cent, mile)
        print(p_print)
    else:
